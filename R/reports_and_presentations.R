...
create_bioinfo_html <- function(file_name = NULL,
  ...
  # copy from internals
  file.copy(
    from = system.file("extdata", "_extensions/", ext_name, package = "IMBEIstyle"),
  ...
  source_file_qmd <- file.path(report_folder, paste0(file_name, ".qmd"))
  usethis::edit_file(source_file_qmd)

  cli::cli_alert_success(
    paste0("Have fun with your report - available inside the folder ", report_folder))

  return(invisible(source_file_qmd))
}
...
#' @param file_name Character string, the name of the file for the report -
#' no need to specify the qmd extension.
#' If preparing a presentation that will end up getting published online, it might
#' be clever to rename the rendered file into `index.html` (or start already
#' having an index.qmd file, but in the end it is a good practice to keep
#' informative file names)
#' @param presentation_folder Character string, pointing to the folder where the
#' presentation and its associated files should be created. Defaults to NULL, which
#' falls back to the current working directory.
#' @param ext_name Character string, defaulting to "bioinfo-revealjs". Other values
#' (possibly in other functions) would be "bioinfo-html" or similar...
#' @param expose_aux_files Logical value, whether to expose&copy the auxiliary
#' files (scss, and possibly a .bib file as well) into the toplevel of the
#' specified folder.
#'
#' @returns Invisibly, the path to the created quarto source file for the
#' presentation
#'
#' @export
#'
...
create_bioinfo_revealjs <- function(file_name = NULL,
    ...
    if (is.null(presentation_folder)) {
      ...
      if(!dir.exists(presentation_folder)) {
        ...
        cli::cli_alert_info(paste0("Created presentation folder into ", presentation_folder))
      } else {
        cli::cli_alert_info(paste0("Using presentation folder, ", presentation_folder))
      }
    }

    # check for available extensions
    stopifnot("Extension not in package" = ext_name %in% c("bioinfo-revealjs"))

    # check for existing _extensions directory
    if(!dir.exists(file.path(presentation_folder, "_extensions"))) {
      dir.create(file.path(presentation_folder, "_extensions"))
      cli::cli_alert_info(paste0("Created '_extensions' folder into ", presentation_folder))
    }

    # create folder for the specific extension
    if(!dir.exists(file.path(presentation_folder, "_extensions/", ext_name)))
      dir.create(file.path(presentation_folder, "_extensions/", ext_name))

    # copy from internals
    file.copy(
      from = system.file("extdata", "_extensions/", ext_name, package = "IMBEIstyle"),
      to = file.path(presentation_folder, "_extensions/"),
      overwrite = TRUE,
      recursive = TRUE,
      copy.mode = TRUE
    )
    ...
    # logic check to make sure extension files were moved
    n_files <- length(list.files(file.path(presentation_folder, "_extensions/", ext_name)))
    ...
    # create new qmd report based on skeleton
    file.copy(
      file.path(presentation_folder,
                "_extensions",
                "bioinfo-revealjs",
                "bioinfo-revealjs-presentation-template.qmd"),
      file.path(presentation_folder, paste0(file_name, ".qmd")))

    if (expose_aux_files) {
      # file.copy(file.path(presentation_folder, "_extensions", "bioinfo-revealjs", "references.bib"),
                # file.path(presentation_folder, "references.bib"))
      file.copy(
        file.path(presentation_folder,
                  "_extensions",
                  "bioinfo-revealjs",
                  "ummz_reveal_theme.scss"),
                file.path(presentation_folder, "ummz_reveal_theme.scss"))
      cli::cli_alert_info(
        paste0(
    ...
    # open the new file in the editor
    source_file_qmd <- file.path(presentation_folder, paste0(file_name, ".qmd"))
    usethis::edit_file(source_file_qmd)

    cli::cli_alert_success(
      paste0("Have fun with your presentation - available inside the folder ", presentation_folder))
